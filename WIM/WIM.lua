...
function WIM_SetWindowProps(theWin)
	...
	getglobal(theWin:GetName().."ScrollingMessageFrame"):SetFont("Interface\\AddOns\\\Fonts\\FRIZQT__.TTF",WIM_Data.fontSize);
...
function WIM_Icon_DropDown_Update()
	...
	if(tCount == 0) then
		...
		getglobal("WIM_ConversationMenuTellButton1"):SetText("|cffffffff - 无 -");
...
hooksecurefunc("CastSpellByName", function(spell)
	...
	if spellName=="召唤仪式" then
		--WIM_Summoning_Loading = true;
	end
end)
...
function WIM_ShorcutButton_Clicked()
	...
	elseif(cmd == "summoning") then
		CAPSLOCK_OpenConfigurationDialog();
		CAPSLOCK_AddToSummonQueue(theUser, true);
--		local summoningFind = false;
--		summoningFind = false;
--		for i=0,table.getn(WIM_Summoning_List) do
--			if WIM_Summoning_List[i+1] == theUser then
--				summoningFind = true;
--				table.remove(WIM_Summoning_List,i);
--				break;
--			end;
--		end;
--		if summoningFind then
--			local tmp = 0;
--			for i=0,4 do
--				for j=1,23 do
--					local tmpLink=GetContainerItemLink(i,j);
--					if tmpLink and strfind(tmpLink,"灵魂碎片") then
--						tmp = tmp + 1;
--					end
--				end
--			end
--			TargetByName(theUser, true);
--			CastSpellByName("召唤仪式",theUser)
--			SendChatMessage("当前召唤：["..theUser.."]，剩余片["..(tmp-1).."]，伸出你们的双手点下门",s);
--			SendChatMessage("正在拉你，注意不要进入战斗","WHISPER","通用语",UnitName('target'));
--			if table.getn(WIM_Summoning_List)==0 then
--				SendChatMessage("要拉单M术士，收到后会按顺序拉，不接受YY、团队打字，感谢支持",s);
--			else
--				SendChatMessage("排队等待："..table.concat(WIM_Summoning_List,"、"),s);
--			end;
--		else
--			table.insert(WIM_Summoning_List, theUser);
--			SendChatMessage("排队顺位["..(table.getn(WIM_Summoning_List)).."]，请耐心等待并留意是否有邪恶双子Debuff，若长时间未拉到重新M","WHISPER","通用语",theUser);
--			if table.getn(WIM_Summoning_List)==0 then
--				SendChatMessage("排队等待：无",s);
--			else
--				SendChatMessage("排队等待："..table.concat(WIM_Summoning_List,"、"),s);
--			end;
--		end
